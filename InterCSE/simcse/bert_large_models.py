...
class ClassifierLayer(nn.Module):
    """
    Head for getting classifier over RoBERTa/BERT's CLS representation.
    """
...
def cl_init(cls, config):
    ...
    cls.clssifier = ClassifierLayer(config)
...
def cl_forward(cls,
    ...
    if num_sent >= 5:
        ...
        z4_ori = z4
        z5_ori = z5
        z4 = nn.Softmax(dim=1)(z4)
        z5 = nn.Softmax(dim=1)(z5)
        # z4_z5 = z4 - z5
        # z4_z5 = z4_z5[:, 1]
        bt = z4.size()[0]
        y = [1] * bt
        ...
        # pairwise ce loss
        # z4z5 = torch.cat((z4, z5), 0) 
        z4z5 = torch.cat((z4_ori, z5_ori), 0) 
        bt = z4.size()[0]
        pos_label = [1] * bt
        pos_label = torch.Tensor(pos_label)
        pos_label.view(bt, -1)
        pos_label = pos_label.long().to(cls.device)
        neg_label = [0] * bt
        neg_label = torch.Tensor(neg_label)
        neg_label.view(bt, -1)
        neg_label = neg_label.long().to(cls.device)
        ...
        y.view(bt, -1)
        y = y.long().to(cls.device)
        loss_fct_rank = nn.MarginRankingLoss() 
        pairwise_ranking_loss = loss_fct_rank(z3[:, 1], z4[:, 1], y)
        loss = loss + pairwise_ranking_loss
        # pairwise ce loss
        z3z4 = torch.cat((z3, z4), 0) 
        bt = z3.size()[0]
        pos_label = [1] * bt
        pos_label = torch.Tensor(pos_label)
        pos_label.view(bt, -1)
        ...
        pairwise_ce_loss = loss_fct(z3z4, pos_neg_label)
